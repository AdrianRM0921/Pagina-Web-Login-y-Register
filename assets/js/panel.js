...
async function loadUsers() {
    try {
        ...
        if (auth.currentUser) {
            ...
            const userDoc = users.find(user => user.uid === auth.currentUser.uid);
            userName.textContent = userDoc ? userDoc.usuario : 'Usuario';
            userEmail.textContent = userDoc ? userDoc.correo : 'Correo';
        }
    } catch (error) {
